        ...
        <table class="dashboard-table min-w-full">
            ...
            <tbody class="divide-y divide-brand-50/80 dark:divide-zinc-800">
                @foreach ($customers as $customer)
                    ...
                    <tr class="transition hover:bg-brand-50/45 dark:hover:bg-zinc-800/50 cursor-pointer" wire:click="showItem({{ $customer->id }})">
                        <td>
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="truncate font-black text-zinc-900 dark:text-zinc-100">{{ $customer->username }}</p>
                                    <p class="text-xs font-semibold text-zinc-500 dark:text-zinc-400">#{{ $customer->id }} • {{ optional($customer->created_at)->locale('ar')->translatedFormat('d M Y') }}</p>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="space-y-1">
                                <p dir="ltr" class="text-left font-semibold text-zinc-700 dark:text-zinc-300">{{ $customer->email }}</p>
                                <p dir="ltr" class="text-left text-xs font-semibold text-zinc-500 dark:text-zinc-400">{{ $customer->phone_number ?: 'لا يوجد رقم مسجل' }}</p>
                            </div>
                        </td>
                        <td class="max-w-xs whitespace-normal">
                            <p class="font-black text-zinc-900 dark:text-zinc-100">{{ $profile?->title ?: 'بدون مسمى وظيفي' }}</p>
                            <p class="mt-1 text-xs font-semibold text-zinc-500 dark:text-zinc-400">{{ $profile?->college ?: 'بدون جهة تعليمية' }}</p>
                            <p class="mt-3 text-sm leading-7 text-zinc-600 dark:text-zinc-400">{{ $customer->description ?: 'لا يوجد وصف مختصر.' }}</p>
                        </td>
                        <td>
                        ...
                        <td>
                            <span class="inline-flex items-center rounded-full px-3 py-2 text-xs font-black {{ $customer->is_verified ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' }}">
        ...
        <x-dashboard.detail-modal>
            @if ($this->selectedItem)
                <x-slot:header>
                    ...
                    <p class="mt-1 text-sm font-semibold text-zinc-500 dark:text-zinc-400">عميل مسجل • #{{ $this->selectedItem->id }}</p>
                ...
                <div class="border-t border-brand-100/80 pt-8 dark:border-zinc-800">
                    ...
                    <div class="grid grid-cols-2 gap-y-8 text-center">
                        ...
                        <div>
                            <p class="text-sm font-bold text-zinc-900 dark:text-zinc-100">عدد المشاريع</p>
                            <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">{{ $this->selectedItem->projects_count }} مشروع</p>
                        </div>
                        <div>
